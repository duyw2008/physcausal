...
## v1.4.6 (2026-08-29) — 新机器迁移 + 多核落地: derive 池 16 核并行

硬件换代 (08-28 到货 32 核/32G), 脑从旧机整体迁移到 `~/Agent/physcausal/`。

- **多核落地**: derive 池 16 核并行 (必须 fork; Python 3.14 默认 forkserver 在池里崩), cell_shelf 降频写盘。代速 23s/代 ≈ 旧机 3.2 倍 (旧机 78s)
- **迁移**: 路径统一 `~/Agent/physcausal`, systemd-run transient unit (feynman-brain.service), autostart.lua 登录自启, 回滚包 feynman.tar.gz
- **快照兜底修复**: 恢复时 T4 存活期基线预填充必须用恢复时刻代 (曾用 last_fired 兜底二次误杀 6.5 万弱边)
- 附: 旧电脑未推工作 (reports/ 论文报告族 + evo_colony 更新) 随迁移一并入库

## v1.4.5 (2026-08-27) — QFT 书加料: 白名单 24 定律 + 多方程身份增强

用户 QFT 教育书《一读就懂的量子场论》(6717 行, 8 部分) 的核心因果链注入 (给路径不给结论, 造物主原则)。

- **白名单加料** (`physics/qft_book_laws.py`): 24 定律 / 75 因果方向 / 48 概念进 legal 集。书因果链当种子: QM 局限→场→量子化→相互作用→重整化→规范场论→路径积分→前沿 (诺特定理/规范原理/希格斯机制/重整化群)。验证: 概念全进白名单, 闸门通过, 碎片仍拒, 现有不破坏 (592 = 568+24)
- **多方程身份增强**: 关键概念跨独立定律绑定 (lagrangian_density 4 定律, gauge_field 5, quantum_field 7) — 脑通过「同一概念在多个独立方程中的角色」收敛意义 (多元约束收敛深层语义, 词向量类比), 比文本描述更接近人脑学法
...
- **事故**: `_predictive_feedback` 对数千 reverse_candidates 逐个跑 sympy solve, 无数量上限 → CPU 100% 7 小时无进展 (gen 33731 卡死)
- **修复**: 处理上限 100 (sorted 降序取最有价值) + `_solve_with_timeout` SIGALRM 10s 硬超时 (sympy 无限循环 try/except 捕获不了)
- 验证: 正常 solve 回归 / 超时链路 / 5000 候选只调 100 次

### 学习成果评估 (commit d32b982)
- `scripts/feynman_learning_report.py`: 三层评估 (L1 知识获取: 进图率/入脑率; L2 能力涌现: 结构/变形/SETTLE; L3 理解: 库外发现)
- **变形/发现比** 判据: DERIVE 代数变形零新信息, 占比下降 + 库外发现上升 = 理解萌芽

### 近期关键改动摘要
- **快照 gzip** (8044faa): kg/neural 分离压缩, 1GB→83MB (12x), 序列化峰值 1.5G→300M
- **RPE 隐式预言回路** (6c880d0): 预测是行走副产物, 落空才产生信号 (+2.0 意外命中 / 0 兑现 / 0 落空); 低度节点发现奖励永动机修复 (0b843e8)
- **学会奖励 v3** (7b3659c): 主流预测翻转 + 证据门槛 + 自环不奖, 刷分三轮收敛 (94次/代)
- **enrich 注入修复** (21a1465): 213 条 enrich 因果边从未进图 → 吸收率 69% 根因, `_inject_enrich_edges` init+恢复各调
- **深层因果律种子** (4cd5776/76de8e0): 规范/时间箭头/光锥/熵力 + 类空因果性→自旋统计 (Pauli 1940)
- **LLM 残留清理** (2bc8d1d): arXiv 词共现 63 条注入边清除 + δS=0 白名单过滤
